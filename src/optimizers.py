...
class MomentumSGD(Optimizer):
    def __init__(self, learningrate=0.01, momentum=0.9, params=None):
        super(MomentumSGD, self).__init__(params=params)
        self.learningrate = learningrate
        self.momentum = momentum
        self.vs = [buildsharedzeros(t.shape.eval(), 'v') for t in self.params]
    ...
    def updates(self, loss=None):
        ...
        for v, param, gparam in zip(self.vs, self.params, self.gparams):
            _v = v * self.momentum
            _v = _v - self.learningrate * gparam
...
class SGD(Optimizer):
    def __init__(self, learningrate=0.01, params=None):
    ...
    def updates(self, loss=None):
        ...
        for param, gparam in zip(self.params, self.gparams):
            self.updates[param] = param - self.learningrate * gparam

        return self.updates


class AdaGrad(Optimizer):
    def __init__(self, learningrate=0.01, eps=1e-6, params=None):
        super(AdaGrad, self).__init__(params=params)

        self.learningrate = learningrate
        self.eps = eps
        self.accugrads = [buildsharedzeros(t.shape.eval(),'accugrad') for t in self.params]
    ...
    def updates(self, loss=None):
        ...
        in zip(self.accugrads, self.params, self.gparams):
            ...
            dx = - (self.learningrate / T.sqrt(agrad + self.eps)) * gparam
...
class RMSprop(Optimizer):
    def __init__(self, learningrate=0.001, alpha=0.99, eps=1e-8, params=None):
        ...
        self.mss = [buildsharedzeros(t.shape.eval(),'ms') for t in self.params]
...
class AdaDelta(Optimizer):
    def __init__(self, rho=0.95, eps=1e-6, params=None):
        ...
        self.eps = eps
        self.accugrads = [buildsharedzeros(t.shape.eval(),'accugrad') for t in self.params]
        self.accudeltas = [buildsharedzeros(t.shape.eval(),'accudelta') for t in self.params]
...
class Adam(Optimizer):
    def __init__(self, alpha=0.001, beta1=0.9, beta2=0.999, eps=1e-8, gamma=1-1e-8, params=None):
        ...
        self.ms = [buildsharedzeros(t.shape.eval(), 'm') for t in self.params]
        self.vs = [buildsharedzeros(t.shape.eval(), 'v') for t in self.params]
